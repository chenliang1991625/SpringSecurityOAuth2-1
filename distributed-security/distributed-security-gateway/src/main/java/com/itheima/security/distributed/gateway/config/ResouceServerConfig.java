...
public class ResouceServerConfig  {
    ...
    public class OrderServerConfig extends ResourceServerConfigurerAdapter {
        ...
        public void configure(HttpSecurity http) throws Exception {
            http
                    .authorizeRequests()
//OrderServerConfig指定了若请求匹配/order/**，也就是访问统一用户服务，接入客户端需要有scope中包含read，并且authorities(权限)中需要包含ROLE_API。
//                  测试该order资源服务时打开注释开启,并在该资源服务的资源配置中修改配置
                    .antMatchers("/order/**").access("#oauth2.hasScope('ROLE_ADMIN')");
//                  测试和开发环境使用swagger时打开注释开启,并注释下上面一行
//                    .antMatchers("**/swagger-ui.html","/order/**").permitAll();//网关可以请求swagger API测试页面和/order/**路径的distributed-security-order资源
        }
    }
    //配置其它的资源服务..
}
